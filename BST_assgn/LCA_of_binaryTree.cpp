...
bTreeNode<int>* LCA(bTreeNode<int>* root, int n, int m) {
    if (root == NULL) {
        return NULL;
    }
    ...
    if (search(n, root->left) && search(m, root->left)) {
        ans = LCA(root->left, n, m);
    } else if (search(n, root->right) && search(m, root->right)) {
        ans = LCA(root->right, n, m);
    }
...
int main() {
    ...
    if (!search(n, root) && !search(m, root)) {
        ...
        bTreeNode<int>* output = LCA(root, n, m);
        cout << output->data;
    }
}